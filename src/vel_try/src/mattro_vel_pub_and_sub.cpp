...
public:
  ...
  , m_init_update ( false )
  {
    m_rovo2.open ( "can0" );
    m_rovo2.start();
    subscription_ = this->create_subscription<sensor_msgs::msg::Joy>("joy", 10, std::bind(&VelocitySubscriber::vel_cmd_subscriber_callback, this, _1));
    ...
    publisher_2 = this->create_publisher<geometry_msgs::msg::Twist>("cmd_vel", 10);
    this->declare_parameter<double>("scale_v", 1.0);
    this->declare_parameter<double>("scale_omega", 1.0);
  }
...
private:
  ...
  void topic_callback ( const double vr, const double vl, inmach::mattro::timestamp_type const& ts )
  {
    if ( m_init_update )
    {
      ...
      odometry.pose.pose.orientation.w = std::cos(theta/2.0);
      odometry.pose.pose.orientation.x = 0.0;
      odometry.pose.pose.orientation.y = 0.0;
      odometry.pose.pose.orientation.z = std::sin(theta/2.0);
  ...
  {    
    ...
    velocity.linear.x = v_wish;
    velocity.angular.z = omega_wish;
    publisher_2->publish(velocity);
    
    m_rovo2.set_velocity(vr_wish,vl_wish);
  }

  void vel_cmd_subscriber_callback(const sensor_msgs::msg::Joy::SharedPtr joystick)
  {
    ...
    {
      this->get_parameter("scale_v", scale_v);
      this->get_parameter("scale_omega", scale_omega);

      v_wish = joystick->axes[1] * scale_v;
      omega_wish = joystick->axes[2] * scale_omega;

      vl_wish = v_wish - omega_wish*m_wheel_base/2.0;
      vr_wish = v_wish + omega_wish*m_wheel_base/2.0;
    }
  }
